  ...
  return (
    <div className={classNames('withdrawal-history-item mb-10', props.status.toLowerCase())}>
      <div className="withdrawal-info ">
        <div className="withdrawal-item__icon d-none d-sm-flex px-6">
        ...
        <div className="withdrawal-item__status px-6">
          ...
          <div className="item-cel__content">
            <Svg href={getIconHref(props.status)} height={18} className="d-block d-sm-none mr-3"></Svg>
            {props.status}
          </div>
        </div>
      </div>
      ...
      {(props.status === ETaskStatus.success || props.status === ETaskStatus.inProgress) && !!props.items?.length && (
        <div className="withdrawal-itemsTable mt-4">
          <div className="withdrawal-table__row header d-none d-sm-flex">
          ...
          {props.items?.map((item, i) => (
            <div key={i} className="withdrawal-table__row content flex-column flex-sm-row">
              <div className="withdrawal-cell px-6 px-sm-0">
                <div className="withdrawal-cell__label d-block d-sm-none">{t('Date')}</div>
                {item.issueDate.format('MM.DD.YYYY')}
              </div>
              <div className="withdrawal-cell px-6 px-sm-2">
                <div className="withdrawal-cell__label d-block d-sm-none">{t('Reference')}</div>
                {item.reference}
              </div>
              <div className="withdrawal-cell px-6 px-sm-2">
                <div className="withdrawal-cell__label d-block d-sm-none">{t('Payment Method')}</div>
                {item.paymentMethod}
              </div>
              <div className="withdrawal-cell px-6 px-sm-0">
                <div className="withdrawal-cell__label d-block d-sm-none">{t('Amount')}</div>
                <div>
          ...
          <div className="withdrawal-table__row footer py-5 py-sm-0 px-6 px-sm-0 d-block d-sm-flex">
            {t('Total Amount')}
