			...
			return (
				<section className={classNames('take-control-of-trades-section', props.className)} ref={ref}>
					<Container>
						<Row>
							<Col xs={12}>
								<div className="take-control-of-trades-section__title mb-12">Take Control of Your Trades</div>
							</Col>
						</Row>
							...
							{props.data.map((item, i) => (
								<Col xs={10} md={5} lg={4} key={i} className="px-0 px-lg-7 mx-5 mx-lg-0">
									<div className={classNames('take-control-of-trades-section__item p-9', i % 2 === 0 && 'even')}>
										<div className="item-title mb-4">{item.title}</div>
										<div className="item-description mb-4">{item.desc}</div>
										<Img src={item.img} />
									</div>
								</Col>
							))}
