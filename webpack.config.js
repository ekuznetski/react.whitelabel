...
module.exports = (_env, arguments) => {
	const env = { PRODUCTION: false, LABEL: '', ..._env };
	const targetLabel = env.LABEL || '';
	const targetLabelAssetFolder = targetLabel.length ? `_${targetLabel}` : '_default';
	const excludeAssets = [];
	...
	const targetLabelEnvPath = path.join(
		__dirname,
		`src/domain${targetLabel.length ? '/_' + targetLabel : ''}/env.config.json`,
	);
	...
	let targetLabelAlias = {};
	let targetLabelScss = [];

	// Generate map to replace files for different domain
	if (targetLabel.length) {
		const filenames = fs.readdirSync(`./src/domain/_${targetLabel}`);
		targetLabelAlias = filenames
			...
			.reduce((acc, file) => Object.assign({}, acc, { [`./_default/${file}`]: `./_${targetLabel}/${file}` }), {});

		targetLabelScss = filenames.filter(
	...
	return {
								...
								additionalData: (content, loaderContext) => {
									...
									if (targetLabel.length) {
										...
										_targetLabelCustomizationScssFiles.forEach((scssFileName) => {
											if (targetLabelScss.includes(scssFileName)) {
												const fileName = scssFileName.split('.').slice(0, -1).join('.');
												const relativePath = path
													.relative(
														path.dirname(resourcePath),
														path.join(rootContext, `domain/_${targetLabel}/${fileName}`),
													)
													.replace(/[\\/]/g, '/');
		...
		plugins: [
			new CopyPlugin({
				patterns: [
					{
						from: `locale/${targetLabel.length ? `_${targetLabel}/` : ''}*.json`,
						to: 'locale/',
						flatten: true,
					...
					},
					{
						...
						globOptions: {
							ignore: [
								...
								...(targetLabel.length ? [`**/${targetLabelAssetFolder}/**`] : []),
							],
						},
					},
					...(targetLabel.length
